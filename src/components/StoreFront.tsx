...
import { 
  ShoppingBag, Search, X, ChevronRight, ChevronLeft, 
...
export default function StoreFront({ onOpenAdmin, cart, setCart }: StoreFrontProps) {
  ...
  const handlePlaceOrder = async (e: React.FormEvent) => {
    ...
    if (!customerName || !customerPhone || !customerAddress) {
      alert("الرجاء ملء الحقول المطلوبة (الاسم الكامل، رقم الهاتف، العنوان)");
      return;
    }
    ...
    try {
      ...
      alert("حدث خطأ أثناء إتمام الطلب، الرجاء المحاولة مرة أخرى.");
    } finally {
        ...
        <div className={`grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-6 ${theme.bg.card} p-5 rounded-2xl border ${theme.border.base} ${theme.shadow.soft} text-center sm:text-right`}>
          <div className="flex flex-col sm:flex-row items-center gap-3 p-2">
            ...
            <div>
              ...
              <p className="text-[10px] sm:text-xs text-brand-text-secondary mt-0.5">شحن آمن لجميع محافظات العراق بأجور مدعومة</p>
            </div>
          </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="aspect-[3/4] bg-stone-100 relative overflow-hidden flex-shrink-0">
                    <img
                      src={p.images[0] || "https://images.unsplash.com/photo-1595777457583-95e059d581b8?w=500"}
                      alt={p.name}
                      referrerPolicy="no-referrer"
            ...
            >
              ...
              <div className="w-full md:w-1/2 p-4 sm:p-6 bg-stone-50/50 flex flex-col gap-3">
                <div className={`aspect-[3/4] rounded-2xl overflow-hidden bg-white border ${theme.border.base} shadow-inner relative`}>
                  <img
                    src={selectedProduct.images[activeImageIdx] || "https://images.unsplash.com/photo-1595777457583-95e059d581b8?w=500"}
          ...
          <div className="fixed inset-0 z-50 overflow-hidden">
            ...
            <div className="absolute inset-y-0 left-0 max-w-full flex">
              ...
              >
                      ...
                      {cart.map((item, idx) => (
                        ...
                        >
                          ...
                          <div className="w-16 h-20 bg-stone-200 rounded-lg overflow-hidden flex-shrink-0">
                            <img
                              src={item.product.images[0] || "https://images.unsplash.com/photo-1595777457583-95e059d581b8?w=500"}
                ...
                {cart.length > 0 && (
                  <div className={`p-6 border-t ${theme.border.base} ${theme.bg.secondary} text-right`}>
                    <div className="flex items-center justify-between text-brand-text mb-4">
                      <span className="text-xs sm:text-sm">المجموع الفرعي للمشتريات:</span>
                      <span className="text-base sm:text-lg font-black text-brand-primary">
                        {formatPrice(cartTotal)}
                      </span>
                    </div>

                    <p className="text-[10px] text-brand-text-secondary mb-4 leading-normal">
                      * يرجى إتمام الطلب لتعبئة تفاصيل التوصيل واختيار طريقة الدفع المناسبة لكِ.
                    </p>
                    ...
                    <button
                      ...
                      className="w-full py-3.5 bg-brand-primary hover:bg-brand-primary-hover text-white text-sm sm:text-base font-bold rounded-full shadow-lg transition-all text-center flex items-center justify-center gap-2 transform hover:scale-[1.01]"
                    >
                      متابعة الشراء وتثبيت الطلب 💳
                    </button>
                  </div>
                )}
