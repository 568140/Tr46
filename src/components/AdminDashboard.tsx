...
export default function AdminDashboard({ onClose }: AdminDashboardProps) {
  ...
  const handleAutoImportFromUrl = async () => {
    if (!importUrl) {
      alert("الرجاء إدخال رابط المنتج المراد استيراده أولاً");
      return;
    }
    try {
      ...
      alert("✨ تم استيراد المنتج بنجاح وجرت مزامنته تلقائياً إلى قاعدة البيانات!");
      setIsProductFormOpen(false);
      setImportUrl("");
      setImportMode('local');
    } catch (e) {
      console.error(e);
      alert("حدث خطأ أثناء الاستيراد والمزامنة التلقائية");
    }
  };
  ...
  const handleSaveProduct = async (e: React.FormEvent) => {
    ...
    if (!productForm.name || productForm.price <= 0) {
      alert("الرجاء ملء حقل الاسم والسعر.");
      return;
    }
  ...
  const handleSaveWallet = async (e: React.FormEvent) => {
    ...
    if (!walletForm.name || !walletForm.numberOrAddress) {
      alert("الرجاء ملء اسم المحفظة والتعليمات.");
      return;
    }
  ...
  const handleSaveBanner = async (e: React.FormEvent) => {
    ...
    if (!bannerForm.title || !bannerForm.imageUrl) {
      alert("الرجاء ملء العنوان ورابط الصورة.");
      return;
    }
  ...
  const handleSaveNotif = async (e: React.FormEvent) => {
    ...
    if (!notifForm.title || !notifForm.message) {
      alert("الرجاء ملء العنوان ونص الإشعار.");
      return;
    }
  ...
  const handleSaveUser = async (e: React.FormEvent) => {
    ...
    if (!userForm.username || !userForm.displayName || !userForm.passwordHash) {
      alert("الرجاء ملء كافة حقول الحساب.");
      return;
    }
    ...
    try {
      if (editingUser) {
        ...
        alert("تم تعديل حساب الموظف/المشرف بنجاح!");
      } else {
  ...
  const handleDeleteDoc = async (collectionName: string, id: string) => {
    if (confirm("هل أنتِ متأكدة من رغبتكِ في حذف هذا العنصر نهائياً من قاعدة البيانات؟ لا يمكن التراجع عن هذه الخطوة.")) {
      try {
        ...
        {isWalletFormOpen && (
          <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
              className={`bg-white rounded-3xl p-6 sm:p-8 max-w-md w-full text-right relative z-10 border border-brand-border shadow-2xl space-y-4`}
            >
              <h3 className="text-base sm:text-lg font-bold text-brand-primary border-b border-brand-border/40 pb-2">
                {editingWallet ? "📝 تعديل المحفظة الإلكترونية" : "💳 إضافة محفظة دفع جديدة"}
              </h3>
            ...
            >
              ...
              <form onSubmit={handleSaveUser} className="space-y-4 text-xs sm:text-sm">
                ...
                <div>
                  ...
                  <input
                    type="text" // keep as text so admin can see and edit passwords clearly as requested! "تعديل كلمات المرور والصلاحيات"
                    required
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block font-bold text-brand-text mb-1">الصلاحية الوظيفية *</label>
                    <select
                      value={userForm.role}
                      disabled={editingUser?.id === 'user-admin'} // main admin is always admin
                      onChange={(e) => setUserForm({ ...userForm, role: e.target.value as any })}
                      className={`w-full px-4 py-2 rounded-xl border border-brand-border cursor-pointer`}
                    >
                      <option value="admin">مدير عام (Full Access) 👑</option>
                      <option value="sales">مسؤول مبيعات (Sales Supervisor) ⚙️</option>
                      <option value="cs">خدمة عملاء (Customer Service) 💬</option>
                    </select>
                  </div>

                  <div className="flex items-center gap-2 pt-5 font-bold cursor-pointer text-brand-text-secondary">
                    <input
                      type="checkbox"
